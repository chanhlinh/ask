...
class QuestionModerationController extends Controller {
 ...
 public function update(Request $r,Event $event,Question $question){abort_unless($question->event_id===$event->id,404);$status=$r->validate(['status'=>['required',Rule::in([Question::PENDING,Question::APPROVED,Question::ANSWERED,Question::REJECTED])]])['status'];$question->update(['status'=>$status]);broadcast(new QuestionChanged($question->fresh()));return ['question'=>$this->data($question->fresh())];}
